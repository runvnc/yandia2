...
# Default to localhost, override with: SERVER=https://your-url ./test_server.sh
...
# Test TTS with the example prefix files (relative paths)
...
curl -X POST "$SERVER/tts" \
  ...
  -F "speaker_1=example_prefix1.wav" \
  -F "speaker_2=example_prefix2.wav" \
  -F "cfg_scale=2.0" \
  -F "temperature=0.8" \
